...
python main.py --run-now --slot 1   # slot 0=morning, 1=afternoon, 2=evening
...
| `POSTING_TIMES` | Comma-separated HH:MM times, default `08:00,14:00,20:00` |
...
**Niche rotation**: `config.py::NICHES` defines 4 niches. The active niche is selected by `_pick_niche(slot)` using `(day_of_year * 3 + slot) % len(NICHES)`.
...
**CI/CD**: `.github/workflows/post_shorts.yml` triggers via cron at 08:00, 14:00, 20:00 UTC. Can also be triggered manually via `workflow_dispatch` with a `slot` input. Uploads `result.json`, `seo.json`, and `script.json` as artifacts retained for 30 days.
