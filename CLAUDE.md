...
**Posting schedule**: No more fixed cron times. The workflow triggers hourly; `scripts/should_post_now.py` (stdlib-only, runs before dependency install to keep the ~21 "skip" hours cheap) decides whether the current hour is worth the full pipeline. It queries Instagram's `online_followers` insight for a real per-account signal once there's enough follower activity (Facebook Insights need 100+ Page likes to return anything at this account size; YouTube has no equivalent for a new channel), and falls back to well-established short-form-video engagement windows (`AUDIENCE_TIMEZONE`, default `America/New_York`, DST-aware via zoneinfo) otherwise. As the IG account grows, it automatically switches to real data — no manual re-tuning. Manual `workflow_dispatch` runs always bypass the check.
